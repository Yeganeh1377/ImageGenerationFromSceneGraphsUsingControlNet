...
# experiment setup
parser.add_argument('--batch_size', default=1, type=int) # default of Justin for testing 3
...
def main(args):
    ...
    pipe.scheduler = UniPCMultistepScheduler.from_config(pipe.scheduler.config) # frank  use this
    ...
    for i in range(len(ids)):
        prompts[str(ids[i])+".png"] = sd_file[i][:-2]   
    #print("prompts of all the test images:\n", prompts)

    ########################## load control images to use for making sure we only look at valid images #######################
    img_paths = {}
    ...
    for key in img_paths.keys(): # so are sure that it only considers those imgs with segment maps
        if not key in prompts: #this should not happen
            continue
        ...
        with torch.autocast("cuda"):
            #print(f"memory data{torch.cuda.max_memory_allocated():,}, {torch.cuda.max_memory_reserved():,}")
            images = pipe(prompt, num_images_per_prompt=args.batch_size, generator=generator, num_inference_steps=args.iters).images
            # save generated images: in reality for quantitative there is only 1 per image
            for i in range(len(images)):
                img_path = os.path.join(args.sd_dir, str(i)+key)
